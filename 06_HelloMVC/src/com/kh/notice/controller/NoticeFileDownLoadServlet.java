...
public class NoticeFileDownLoadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//2. 파일입출력을 위한 스트림 생성
		//   파일 불러올때는 서버에서 inputStream생성
		//   파일을 보낼때는 : client -> inputStream에서 스트림 받아서 처리
		//3. 파일에 대한 인코딩처리(브라우저에 따른 인코딩처리)
		//4. 해더환경설정 
		//	  응답 contentType설정(application/octet-stream
		//   Content-Disposition설정 : 파일을 어떻게 받을 것인지, 파일명 설정
		//   attachment(다운로드 팝업 open)/inline(브라우저가 열 수 있는 확장자면 브라우저에서 open)
		//   filename=파일명
		...
		//빠른 속도를 위해 버퍼를 씌운 FileInputStream을 생성
